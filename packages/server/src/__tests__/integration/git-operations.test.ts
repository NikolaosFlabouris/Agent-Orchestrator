...
const FORGEJO_TOKEN = process.env.TEST_FORGEJO_TOKEN ?? '';
...
const SKIP = !FORGEJO_URL || !FORGEJO_TOKEN || process.platform === 'win32';
...
describe.skipIf(SKIP)('Git operations integration', { timeout: 60_000 }, () => {
  ...
  beforeAll(async () => {
    ...
    repoUrl = `${url.protocol}//${FORGEJO_USER}:${FORGEJO_TOKEN}@${url.host}/${FORGEJO_USER}/${testRepoName}.git`;
  ...
  afterAll(async () => {
    ...
    try {
      await fetch(
        ...
        { method: 'DELETE', headers: { Authorization: `token ${FORGEJO_TOKEN}` } }
      );
    } catch { /* */ }
  });
  ...
  it('commits and pushes to agent/* branch', () => {
    ...
    const branches = git(['ls-remote', '--heads', 'origin', 'agent/issue-1-test']);
  ...
  it('fetches remote branch changes', () => {
    ...
    execFileSync('git', ['-c', 'credential.helper=', 'clone', repoUrl, clone2.replace(/\\/g, '/')], {
      encoding: 'utf-8',
      timeout: 30_000,
      stdio: ['pipe', 'pipe', 'pipe'],
    });
    ...
    execFileSync('git', ['-c', 'credential.helper=', 'fetch', 'origin', 'agent/issue-1-test'], {
      cwd: clone2,
      encoding: 'utf-8',
      timeout: 30_000,
      stdio: ['pipe', 'pipe', 'pipe'],
    ...
    execFileSync('git', ['checkout', '-B', 'agent/issue-1-test', 'origin/agent/issue-1-test'], {
      cwd: clone2,
      encoding: 'utf-8',
      timeout: 30_000,
      stdio: ['pipe', 'pipe', 'pipe'],
    ...
    const content = fs.readFileSync(path.join(clone2, 'test-file.txt'), 'utf-8');
