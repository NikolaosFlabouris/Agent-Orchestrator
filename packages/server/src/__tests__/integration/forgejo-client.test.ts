...
const SKIP = !FORGEJO_URL || !FORGEJO_TOKEN;
...
describe.skipIf(SKIP)('ForgejoClient integration', () => {
  ...
  it('comments on an issue', async () => {
    ...
    // Should not throw
    await client.commentOnIssue(repo, issue.number, 'Test comment from integration test');
  });
  ...
  it('creates and manages labels', async () => {
    ...
    // Verify
    const fetched = await client.getIssue(repo, issue.number);
    expect(fetched.labels.some((l) => l.name === 'status/test-label')).toBe(true);
  });
  ...
  it('uses replaceLabelByNames with auto-creation', async () => {
    ...
    await client.replaceLabelByNames(repo, issue.number, ['status/auto-created']);

    const fetched = await client.getIssue(repo, issue.number);
    expect(fetched.labels.some((l) => l.name === 'status/auto-created')).toBe(true);
  });
