...
- **Download** — a plain `<a download>` on `GET /api/tasks/:id/log`, which streams `progress.log` off disk — from the task workspace while it exists, and transparently gunzipped from `/data/archive/` once the retention sweep has removed the workspace (see [Run Artifact Archive](07-deployment-operations.md#run-artifact-archive)). This is the escape hatch for both the filter and the replay tail cap below, so it is deliberately *not* wired to the panel's state.
...
Any path that matches no route renders `NotFound` — the standard dark page shell with `AppHeader`, "Page not found", the path that missed, and a link home. The catch-all lives *inside* the authenticated layout, so an unknown path behaves like every other view for a signed-out visitor (redirected to sign in) rather than leaking a second public page.
...
**Grid items need the same treatment — and so does the track.** A grid item also defaults to `min-width: auto`, which resolves to its content's min-content width, so the rule above applies to every `grid` cell: give it `min-w-0` when it holds text that must truncate. Grids have a second, less obvious failure the flex rule has no counterpart for — a `grid` with no `grid-cols-*` puts its items in an implicit `auto` track, and an `auto` track's base size is the widest item's min-content width, with nothing clamping it to the container. That track can therefore be wider than the page whatever its items say, so write `grid-cols-1` (Tailwind emits `minmax(0, 1fr)`) rather than a bare `grid` even when there is only one column. Both halves are needed and neither is sufficient alone: `grid-cols-1` clamps the track, `min-w-0` clamps the item inside it. This is what made a long-titled active task on the dashboard hang off the right edge of the screen at *every* width, desktop included — unlike the flex cases, it is not a small-screen-only bug.
...
**Global base styles live in `index.css`, not in every component.** `@layer base` there sets `color-scheme: dark` on `:root` — that is what makes browser-drawn chrome (`<select>` dropdowns, the Reports date pickers, scrollbars) render dark instead of light — and a single `:focus-visible` outline so keyboard focus is visible everywhere without adding a `focus:` utility per control. It is `:focus-visible`, not `:focus`, so pointer clicks show nothing. The same file loads `@plugin "@tailwindcss/typography"`, which is what the `prose prose-invert` classes on the Create Task markdown preview need.

**Shared primitives own colour; call sites own size and layout.** Three recipes that were copy-pasted across views now live in `packages/ui/src/components`: `StatusBadge` (the one task-status → colour map, sizes `sm` for rows and `md` for the Task Detail header), `Button` (`primary`, `secondary`, the `danger`/`success`/`warn`/`caution`/`info` outline set from the Task Detail actions bar, and the filled `tonal-*` pair behind the dashboard's Pause/Resume toggle), and `Input`/`Select`/`Textarea` (the `bg-gray-800 border border-gray-700 rounded px-3 py-2 text-sm` field recipe, with a `surface` prop for the `gray-900` fill Create Task uses). Reach for these rather than pasting the classes again. What they deliberately do *not* own is padding, width and the `min-h-11 sm:min-h-0` touch target: those are per-placement, and Tailwind has no reliable last-one-wins override for two conflicting utilities without a merge helper, so they stay in the call site's `className` (every primitive merges one). Badge colours are not the chart colours — `STATUS_COLORS` in `Reports.tsx` is a separate hex map for Recharts bars, which need hues that differ from each other rather than muted pairs that stay readable behind text. Tables and tab bars are intentionally *not* extracted: their columns and semantics are bespoke per view, so a shared component would be configuration, not reuse.
...
Every task payload — in `snapshot`, `task_updated` and `task_created` alike — is the **same enriched object** `GET /api/tasks` returns (`TaskView` in `packages/shared`), produced by the single serializer in `packages/server/src/task-view.ts`. The client replaces a task wholesale when an event arrives, so a leaner payload would silently strip fields (`repo`, `dependencies`/`blocked`, `health`, the resolved agent-profile chains) off rows it already held. Task Detail treats the same frame as a *merge* into its loaded task — the frame is complete for the `TaskView` fields, but the view also holds the detail-only `attempts`/`events`/`forgejo_links` — and pays for a full `GET /api/tasks/:id` only when the frame's `status`, `attempt`, `container_id`, or `pr_number` differs from what it holds (the cases where the detail payload can actually change — `pr_number` is written mid-run with no other gate field moving, and `forgejo_links.pr` exists only in the detail response). The merge also keeps the loaded `container_name` and `has_human_review_label` when the frame's are null, since list/WS frames never resolve the container name and carry a null review-label when the snapshot cache is cold. Queue reorders and profile-edit echoes, which emit a `task_updated` per row write, cost zero refetches (`taskUpdateNeedsRefetch` in `TaskDetail.tsx`).
...
**One connection per tab.** The dashboard socket is owned app-wide by `<LiveData>` (mounted inside `GatedLayout`), not by any single view, and views attach to it through the reference-counted `subscribeDashboard` in `ws.ts`. Navigating Dashboard → Task Detail → Dashboard therefore neither tears the socket down nor opens a second one; the store stays live on Reports and Settings too. Task Detail attaches to the same stream to fold its own task's `task_updated` frames into local state, refetching the full detail only on a status/attempt/container/PR change (see above).
