...
Every task payload — in `snapshot`, `task_updated` and `task_created` alike — is the **same enriched object** `GET /api/tasks` returns (`TaskView` in `packages/shared`), produced by the single serializer in `packages/server/src/task-view.ts`. The client replaces a task wholesale when an event arrives, so a leaner payload would silently strip fields (`repo`, `dependencies`/`blocked`, `health`, the resolved agent-profile chains) off rows it already held. Task Detail treats the same frame as a *merge* into its loaded task — the frame is complete for the `TaskView` fields, but the view also holds the detail-only `attempts`/`events`/`forgejo_links` — and pays for a full `GET /api/tasks/:id` only when the frame's `status`, `attempt`, or `container_id` differs from what it holds (the cases where the detail payload can actually change). Queue reorders and profile-edit echoes, which emit a `task_updated` per row write, cost zero refetches (`taskUpdateNeedsRefetch` in `TaskDetail.tsx`).
...
**One connection per tab.** The dashboard socket is owned app-wide by `<LiveData>` (mounted inside `GatedLayout`), not by any single view, and views attach to it through the reference-counted `subscribeDashboard` in `ws.ts`. Navigating Dashboard → Task Detail → Dashboard therefore neither tears the socket down nor opens a second one; the store stays live on Reports and Settings too. Task Detail attaches to the same stream to fold its own task's `task_updated` frames into local state, refetching the full detail only on a status/attempt/container change (see above).
